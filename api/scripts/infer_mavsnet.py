...
try:
    from api.models.mavsnet_adapter import (
        MAVSNET_ROOT,
        MavsNetAdapter,
        get_fixed_backbone,
    )
except Exception as exc:
    print(f"Error importing MavsNetAdapter: {exc}")
    traceback.print_exc()
    sys.exit(1)
...
def detect_subset(weight_path: Path, subset_arg: str = "") -> str:
    subset = (subset_arg or os.getenv("AVS_MAVSNET_SUBSET", "")).strip().lower()
    if subset in {"s4", "ms3"}:
        return subset
...
def choose_config_path(weight_path: Path, config_path_arg: str = "", subset_arg: str = "") -> Tuple[Path, str, str]:
    if config_path_arg:
        ...
        if not cfg.is_absolute():
            cfg = (MAVSNET_ROOT / cfg).resolve()
        subset = detect_subset(weight_path=weight_path, subset_arg=subset_arg)
        backbone = get_fixed_backbone()
        return cfg, subset, backbone

    subset = detect_subset(weight_path=weight_path, subset_arg=subset_arg)
    backbone = get_fixed_backbone()

    # Prefer MAVS-Net native configs first.
    fallback_candidates = [
        MAVSNET_ROOT / "config" / subset / f"MAVSNet_pvt2_{subset}.py",
        MAVSNET_ROOT / "config" / subset / f"AVSegFormer_pvt2_{subset}.py",
    ]
    for one in fallback_candidates:
        if one.is_file():
            return one, subset, backbone

    raise FileNotFoundError(
        f"No PVT config found for subset={subset}. Expected one of: "
        + ", ".join(str(p) for p in fallback_candidates)
    )
...
def main() -> None:
    started = time.time()
    parser = argparse.ArgumentParser(description="Run MAVS-Net inference")
    ...
    parser.add_argument("--config_path", default="", help="Optional MAVS-Net config path")
    ...
    if not config_path.is_file():
        print(f"Error: MAVS-Net config not found: {config_path}")
        sys.exit(1)
    ...
    print(f"Loading MAVS-Net model from {weight_path} with config {config_path}...")
    try:
        adapter = MavsNetAdapter(config_path=str(config_path))
        adapter.load_weights(str(weight_path), device=runtime_device)

        model_dev = model_parameter_device(adapter.model)
        print(f"[mavsnet] loaded_model_device={model_dev}")
        if runtime_device.startswith("cuda") and not model_dev.startswith("cuda"):
            raise RuntimeError(
                "MAVS-Net model is not on CUDA after load. "
    ...
    print(f"Running MAVS-Net inference on {len(frames)} frames...")
    masks_bytes: List[bytes] = []
    try:
    ...
    report = {
        ...
        "note": f"MAVS-Net inference done with config={config_path.name}; 指标为论文固定值。",
    }
