...
try:
    from api.models.vct_adapter import VCT_ROOT, VctAdapter
except Exception as e:
    print(f"Error importing VctAdapter: {e}")
    traceback.print_exc()
    sys.exit(1)

try:
    # Imported after VctAdapter so VCT_ROOT is already injected into sys.path.
...
def choose_config_path(weight_path: Path, config_path_arg: str = "", subset_arg: str = "") -> Path:
    if config_path_arg:
        p = Path(config_path_arg).expanduser()
        if not p.is_absolute():
            p = (VCT_ROOT / p).resolve()
        return p

    subset = (subset_arg or os.getenv("AVS_VCT_SUBSET", "")).strip().lower()
    if not subset:
        weight_lower = str(weight_path).lower()
        if "ms3" in weight_lower:
            subset = "ms3"
        elif "/ss_" in weight_lower or "ss_swin" in weight_lower:
            subset = "ss"
        else:
            subset = "s4"

    mapping = {
        "s4": VCT_ROOT / "configs" / "s4_swinb_384" / "Test_COMBO_SWINB.yaml",
        "ms3": VCT_ROOT / "configs" / "ms3_swinb_384" / "Test_COMBO_SWINB.yaml",
        "ss": VCT_ROOT / "configs" / "ss_swinb_384" / "Test_COMBO_SWINB.yaml",
    }
    return mapping.get(subset, mapping["s4"])
...
def main():
    started = time.time()
    parser = argparse.ArgumentParser(description="Run VCT inference")
    ...
    parser.add_argument("--config_path", default="", help="Optional VCT config path")
    parser.add_argument("--subset", default="", help="Optional subset in {s4,ms3,ss}")
    ...
    config_path = choose_config_path(weight_path=weight_path, config_path_arg=args.config_path, subset_arg=args.subset)
    if not config_path.exists():
        print(f"Error: VCT config not found: {config_path}")
        sys.exit(1)
    ...
    print(f"Loading VCT model from {weight_path} with config {config_path}...")
    try:
        adapter = VctAdapter(config_path=str(config_path))
        adapter.load_weights(str(weight_path), device=runtime_device)

        model_dev = model_parameter_device(adapter.model)
        print(f"[vct] loaded_model_device={model_dev}")
        if runtime_device.startswith("cuda") and not model_dev.startswith("cuda"):
            raise RuntimeError(
                "VCT model is not on CUDA after load. "
    ...
    subset = (args.subset or "").strip().lower()
    if not subset:
        weight_lower = str(weight_path).lower()
        subset = "ms3" if "ms3" in weight_lower else "s4"
    metrics = estimate_metrics(algorithm="vct", subset=subset, coverage_pct_by_frame=coverage_pct_by_frame)
    report = {
        ...
        "note": f"VCT inference done with config={config_path.name}; 指标为无标注场景估计值。",
    }
