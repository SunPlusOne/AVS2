...
class AVSegHead(nn.Module):
    def __init__(self,
        ...
        if bfm_block is not None:
            self.bfm_block = build_fusion_block(**bfm_block)
    ...
    def forward(self, feats, audio_feat):
        ...
        # COMBO-style bidirectional fusion after visual mixer and before decoder
        if hasattr(self, 'bfm_block'):
            src_flatten, audio_feat = self.bfm_block(src_flatten, audio_feat)

        # prepare prompted visual queries
        bs = audio_feat.shape[0]
        query = self.query_generator(src_flatten, audio_feat)
