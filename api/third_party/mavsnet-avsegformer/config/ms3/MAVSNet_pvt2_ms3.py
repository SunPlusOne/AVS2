model = dict(
    ...
    head=dict(
        ...
        # Keep AVSegFormer mixer while inserting BFM before transformer decode.
        fusion_block=dict(type='CrossModalMixer'),
        bfm_block=dict(type='BidirectionalFusionModule', dim=256, n_heads=8, dropout=0.1),
        query_generator=dict(
            type='PromptedVisualQueryGenerator',
            query_num=300,
            prototype_num=8,
            embed_dim=256,
            num_heads=8,
...
loss = dict(
    weight_dict=dict(
        iou_loss=1.0,
        mix_loss=0.1,
        temporal_consistency_loss=0.05),
    loss_type='bce_dice',
