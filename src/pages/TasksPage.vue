  ...
  <div class="grid gap-4">
    <div class="flex items-end justify-between gap-3">
      <div>
        ...
        <div class="mt-1 text-sm text-zinc-400">本地保存历史任务列表（浏览器 localStorage）</div>
      </div>
      ...
      <el-table :data="rows" size="small" style="width: 100%" row-key="task_id">
        ...
        <el-table-column label="算法" min-width="120">
          <template #default="scope">
            <span class="text-zinc-200">{{ scope.row.algorithm ?? '—' }}</span>
          </template>
        </el-table-column>
        ...
        <el-table-column label="进度" min-width="120">
          <template #default="scope">
            <span class="text-zinc-200">{{ scope.row.progress }}%</span>
          </template>
        </el-table-column>
